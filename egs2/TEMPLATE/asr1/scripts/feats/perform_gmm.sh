...
gmm_dir=             # Directory for the kmeans models
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ] && ! [[ " ${skip_stages} " =~ [[:space:]]2[[:space:]] ]]; then
    log "stage 2: Learn GMM with ${feature_type} feature based on scikit-learn"

    _logdir="${gmm_dir}/logdir"
    mkdir -p ${_logdir}
    ...
    if (( $(echo "${_portion} >= 1.0" | bc -l) )); then
        cp "${featdir}/${feature_type}/${suffix}${_dset}"/feats.scp "${55
        gmm_dir}/train.scp"
    else
        ...
        subset_scp.pl \
            ...
            > "${gmm_dir}/train.scp" || exit 1;
        log "Subsampling ${portion_nutt} utterances for Kmeans training."
    fi

    # It typically requires 120GB RAM to run kmeans steps.
    ${cpu_cmd} --num_threads ${num_threads} ${_logdir}/learn_gmm.log \
        ${python} pyscripts/utils/learn_gmm.py \
            --km_path ${gmm_dir}/gmm_${nclusters}.mdl \
            --n_clusters ${nclusters} \
            --percent -1 \
            --in_filetype mat \
            "scp:${gmm_dir}/train.scp" || exit 1;
fi


if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ] && ! [[ " ${skip_stages} " =~ [[:space:]]3[[:space:]] ]]; then
    log "stage 3: Generate GMM pseudo-labels"
    ...
    for dset in "${train_set}" "${dev_set}" ${other_sets}; do
        ...
        split_scps=""
        for n in $(seq ${_nj}); do
            split_scps+=" ${_dump_dir}/logdir/inference_gmm.${n}.scp"
        done
        # shellcheck disable=SC2086
        utils/split_scp.pl "${key_file}" ${split_scps}
        ...
        ${_cmd} JOB=1:${_nj} "${_dump_dir}"/logdir/inference_pseudo_labels_km${nclusters}.JOB.log \
            ${python} pyscripts/feats/dump_gmm_label.py \
                ...
                "ark,t:${_dump_dir}/logdir/pseudo_labels_gmm${nclusters}.JOB.txt" || exit 1;

        # concatenate scp files
        for n in $(seq ${_nj}); do
            cat "${_dump_dir}"/logdir/pseudo_labels_gmm${nclusters}.${n}.txt || exit 1;
        done | sed 's/ \[ \| \]//g' | sort -u > "${_dump_dir}"/pseudo_labels_gmm${nclusters}.txt || exit 1;
    done
fi


gmm_tag=$(basename ${gmm_dir})
    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ] && ! [[ " ${skip_stages} " =~ [[:space:]]4[[:space:]] ]]; then
        ...
        if [ -d "${alignment_phoneme_dir}" ]; then
            ...
            ${python} local/measure_teacher_quality.py \
                ...
                --lab_name "pseudo_labels_gmm${nclusters}.txt" \
                --lab_sets "${dev_set}" \
                ...
                --upsample ${upsample} \
                --ref_lab_dir "" \
                --ref_lab_name "" | tee ${gmm_dir}/phoneme_pseudo_label_quality.txt
        else
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ] && ! [[ " ${skip_stages} " =~ [[:space:]]5[[:space:]] ]]; then
    ...
    for dset in "${train_set}" "${dev_set}" ${other_sets}; do
        ...
        if [ -f "${label_dir}"/pseudo_labels_gmm${nclusters}.txt ]; then
            cp "${label_dir}"/pseudo_labels_gmm${nclusters}.txt ${datadir}/${dset}/text.gmm.${gmm_tag}
        fi
        utils/fix_data_dir.sh --utt_extra_files "text.gmm.${gmm_tag}" ${datadir}/${dset}
    done
    ...
    if [ -n "${dictdir}" ]; then
        ...
        <${datadir}/${train_set}/text.gmm.${gmm_tag} cut -d" " -f2- | \
