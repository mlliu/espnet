...
stage=2 # stage 1 is uaed to extract wavLM features,and has been done
stop_stage=100
skip_stages=
cpu_cmd="queue.pl" #"run.pl"
num_threads=20      # number of cpu threads in learn_kmeans
cuda_cmd="queue.pl" #"run.pl"
...
featdir=/export/fs05/mliu121/espnet_data/librispeech_100_asr2/dump/extracted   # Directory for the dumped features and labels.
pca_dir=             # Directory for the kmeans models
...
n_components=80       # Number of components in PCA
portion=0.1         # Portion of data from training set used to train pca
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ] && ! [[ " ${skip_stages} " =~ [[:space:]]2[[:space:]] ]]; then
    log "stage 2: Learn PCA with ${feature_type} feature based on faiss"

    _logdir="${pca_dir}/logdir"
    mkdir -p ${_logdir}
    ...
    if (( $(echo "${_portion} >= 1.0" | bc -l) )); then
        cp "${featdir}/${feature_type}/${suffix}${_dset}"/feats.scp "${pca_dir}/train.scp"
    else
        ...
        subset_scp.pl \
            ...
            > "${pca_dir}/train.scp" || exit 1;
        log "Subsampling ${portion_nutt} utterances for Kmeans training."
    fi

    # It typically requires 120GB RAM to run kmeans steps.
    ${cpu_cmd} --num_threads ${num_threads} ${_logdir}/learn_pca.log \
        ${python} pyscripts/utils/learn_pca.py \
            --pca_path ${pca_dir}\
            --n_components ${n_components} \
            --percent -1 \
            --in_filetype mat \
            "scp:${pca_dir}/train.scp" || exit 1;
fi


if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ] && ! [[ " ${skip_stages} " =~ [[:space:]]3[[:space:]] ]]; then
    log "stage 3: perform pca on ${feature_type} feature to reduce dimension to ${n_components}"
    ...
    for dset in "${train_set}"; do # "${dev_set}" ${other_sets}; do
        log "the reduced feature is saved to ${featdir}/${feature_type}/${suffix}${dset}"
        ...
        split_scps=""
        for n in $(seq ${_nj}); do
            split_scps+=" ${_dump_dir}/logdir/inference_pca.${n}.scp"
        done
        # shellcheck disable=SC2086
        utils/split_scp.pl "${key_file}" ${split_scps}
        ...
        for n in $(seq ${_nj}); do
            awk '(FILENAME==ARGV[1]){utt2num[$1]=$2} (FILENAME==ARGV[2]){print($1, utt2num[$1])}' \
                ${datadir}/${dset}/utt2num_samples ${_dump_dir}/logdir/inference_pca.${n}.scp \
                > ${_dump_dir}/logdir/utt2num_samples.${n}
        done

        ${_cmd} JOB=1:${_nj} "${_dump_dir}"/logdir/inference_pseudo_labels_pca${n_components}.JOB.log \
            ${python} pyscripts/feats/dump_pca_transformation.py \
                ${_opts} \
                --pca_path "${pca_dir}/${n_components}_pca" \
                --out_filetype "mat" \
                --use_gpu ${use_gpu} \
                --utt2num_samples "${_dump_dir}/logdir/utt2num_samples.JOB" \
                "scp:${_dump_dir}/logdir/inference_pca.JOB.scp" \
                "ark,scp:${_dump_dir}/logdir/feat_pca${n_components}.JOB.ark,${_dump_dir}/logdir/feat_pca${n_components}.JOB.scp" || exit 1;

        # concatenate scp files
        for n in $(seq ${_nj}); do
            cat "${_dump_dir}"/logdir/feat_pca${n_components}.${n}.scp || exit 1;
        done | sed 's/ \[ \| \]//g' | sort -u > "${_dump_dir}"/feat_pca${n_components}.scp || exit 1;
    done
fi
