...
class ApplyGmm(object):
    def __init__(self, gmm_path, use_gpu):
        self.gmm_model = joblib.load(gmm_path)
        if use_gpu and torch.cuda.is_available():
            self.gmm_model = self.gmm_model.to("cuda")

    def __call__(self, x):
        if isinstance(x, torch.Tensor):
            x = x.to(self.gmm_model.device)
            return self.gmm_model.predict(x).cpu().numpy()
        else:
            return self.gmm_model.predict(x)
...
def dump_label(
    ...
    **kwargs
    ...
    apply_gmm = ApplyGmm(km_path, use_gpu=use_gpu)
        ...
        with file_writer_helper(
            ...
            for utt_ids, data in iterator:
                ...
                for idx, utt in enumerate(utt_ids):
                    lab = apply_gmm(feats[idx][: feats_lens[idx]].numpy())
                    writer[utt] = lab
