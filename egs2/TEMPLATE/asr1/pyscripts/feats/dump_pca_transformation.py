...
class ApplyPCA(object):
    def __init__(self, pca_path, use_gpu):
        self.pca_A = torch.from_numpy(np.load(pca_path + "_A.npy"))
        self.pca_b = torch.from_numpy(np.load(pca_path + "_b.npy"))
        if use_gpu and torch.cuda.is_available():
            self.pca_A = self.pca_A.cuda()
            self.pca_b = self.pca_b.cuda()

    def __call__(self, x):
        if isinstance(x, torch.Tensor):
            x = x.to(self.pca_A.device)
            return torch.matmul(x, self.pca_A) + self.pca_b
        else:
            return np.matmul(x, self.pca_A.cpu().numpy()) + self.pca_b.cpu().numpy()
...
def dump_transformation(
    ...
    **kwargs
    ...
    apply_pca = ApplyPCA(pca_path, use_gpu=use_gpu)
    ...
    if not online_feature_extract:
        ...
        with file_writer_helper(
            ...
            for utt, feat in file_reader_helper(rspecifier, in_filetype):
                trans_feat = apply_pca(feat)
                writer[utt] = trans_feat
                #print("utterance is", utt)
                #print("the original feature shape is: ", feat.shape)
                #print("the transformed feature shape is:", trans_feat.shape)
    else:
        # print error message
        assert False
