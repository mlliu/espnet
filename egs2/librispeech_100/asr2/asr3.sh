...
dataset="librispeech_100" # dataset could be librispeech_100 or swbd or ml-superb or challenge
...
# hmmdir is where the hmm-gmm model is stored
...
# define the input feature and its type for the model
feat1=
feat2=
feat3=
feat4=
...
Options:
    ...
    --skip_4a        # Skip 4a stages (default="${skip_4a}").
...
if [ ${kmeans_feature} = "mfcc" ]; then  # MFCC has no layer
    ...
    #kmeans_feature_conf="{type=s3prl,conf={s3prl_conf=${s3prl_conf},download_dir=/export/fs05/mliu121/espnet_data/ckpt,multilayer_feature=False,layer=${layer}}}"
    s3prl_conf="{upstream=${kmeans_feature_type},upstream_ckpt=/export/fs05/mliu121/espnet_data/ckpt}"
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ] && ! [[ " ${skip_stages} " =~ [[:space:]]3[[:space:]] ]]; then
    ...
    if [ "${feats_type}" = raw ]; then
        log "Stage 3: Format wav.scp: ${datadir}/ -> ${data_audio}"
        ...
        for dset in ${_dsets}; do
            utils/copy_data_dir.sh --validate_opts --non-print ${datadir}/"${dset}" "${data_audio}/${dset}"
            ...
            scripts/audio/format_wav_scp.sh --nj "${nj}" --cmd "${train_cmd}" \
                ...
                "${datadir}/${dset}/wav.scp" "${data_audio}/${dset}"
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ] && ! [[ " ${skip_stages} " =~ [[:space:]]4[[:space:]] ]]; then
    ...
    if ! "${skip_4a}"; then
      ...
      if ! $kmeans_cluster; then
        ...
        if $skip_train; then
          ...
          echo " kmeans_feature_type is ${kmeans_feature_type}  kmeans_feature_conf is ${kmeans_feature_conf}  layer is ${layer}  kmeans_opts is ${kmeans_opts} "
    ...
    if [ "${src_case}" = ts ]; then
	    ...
	    echo "${dset}"
            awk '
                ...
                > "${datadir}/${dset}"/text.${src_case}.${src_lang}
        done
	...
	echo "${dset}"
        ...
        cp ${datadir}/${dset}/text ${datadir}/${dset}/text.${tgt_case}.${tgt_lang}
    done
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ] && ! [[ " ${skip_stages} " =~ [[:space:]]5[[:space:]] ]]; then
    ...
    if [ "${feats_type}" = raw ]; then
        log "Stage 5: ${datadir}/ -> ${data_feats}"
        ...
        for dset in ${_dsets}; do
            ...
            for extra_file in ${utt_extra_files}; do
                ...
                for single_file in ${datadir}/"${dset}"/*; do
...
if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ] && ! [[ " ${skip_stages} " =~ [[:space:]]13[[:space:]] ]]; then
    ...
    if  [ -n "${feat1}" ]; then
        ...
        _opts+="--valid_data_path_and_name_and_type ${_asr_valid_dir}/text.ts.${feat1},src_text_1,text "
        _opts+="--src_token_type_1 ${feat1_type} "
        _opts+="--src_token_list_1  ${token_listdir}/${feat1_type}_${feat1}/src_tokens.txt "
    fi
    if  [ -n "${feat2}" ]; then
      echo "feat2 is not none, and the feat2 is ${feat2}"
        _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/text.ts.${feat2},src_text_2,text "
        _opts+="--valid_data_path_and_name_and_type ${_asr_valid_dir}/text.ts.${feat2},src_text_2,text "
        _opts+="--src_token_type_2 ${feat2_type} "
        _opts+="--src_token_list_2  ${token_listdir}/${feat2_type}_${feat2}/src_tokens.txt "
    fi
    if [ -n "${feat3}" ]; then
        echo "feat3 is not none and the value is ${feat3}"
        _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/text.ts.${feat3},src_text_3,text "
        _opts+="--valid_data_path_and_name_and_type ${_asr_valid_dir}/text.ts.${feat3},src_text_3,text "
        _opts+="--src_token_type_3 ${feat3_type} "
        _opts+="--src_token_list_3  ${token_listdir}/${feat3_type}_${feat3}/src_tokens.txt "
    fi
    if [ -n "${feat4}" ]; then
        _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/text.ts.${feat4},src_text_4,text "
        _opts+="--valid_data_path_and_name_and_type ${_asr_valid_dir}/text.ts.${feat4},src_text_4,text "
        _opts+="--src_token_type_4 ${feat4_type} "
        _opts+="--src_token_list_4  ${token_listdir}/${feat4_type}_${feat4}/src_tokens.txt "
    fi
